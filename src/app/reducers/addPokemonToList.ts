...
import { setToast } from "../slice/AppSlice";
  ...
  async (
    ...
    try {
      ...
      if (index === -1) {
        ...
        return dispatch(
          setToast(`${pokemon.name.toUpperCase()} is added to your collection.`)
        );
      } else {
        return dispatch(
          setToast(
            `${pokemon.name.toUpperCase()} is already a part of your collection.`
          )
        );
      }
    } catch (error) {
